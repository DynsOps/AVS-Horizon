...
export const TemplateManagement: React.FC = () => {
  ...
  const selectAll = () => setForm((prev) => ({ ...prev, permissions: [...allPermissionKeys] }));
  ...
  const deleteTemplate = async (tpl: Template) => {
    ...
    const confirmed = await openConfirmDialog({
      ...
      message: `Delete "${tpl.name}"? Companies currently assigned this template will lose their permissions.`,
      confirmLabel: 'Delete',
      tone: 'danger',
    });
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Create global permission templates and assign them to companies.
          </p>
        </div>
        <button
          onClick={openAdd}
      ...
      <Card title="Global Templates">
        {isLoading ? (
          ...
          <div className="space-y-2">
            ...
            {!templates.length && <p className="text-sm text-slate-500">No global templates yet. Create one to assign to companies.</p>}
          </div>
        )}
      </Card>
          ...
          <div className="relative z-[2147483647] my-2 w-full max-w-2xl rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900 md:my-4">
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-slate-800">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                {modalMode === 'edit' ? 'Edit Template' : 'New Template'}
              </h2>
              <button
                onClick={closeModal}
                className="rounded p-1 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
              ...
              <div className="grid gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Standard Customer Access"
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                      Permissions <span className="ml-1 rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-bold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">{selectedCount}</span>
                    </label>
                  ...
                  <div className="space-y-3 rounded-lg border border-gray-200 p-3 dark:border-slate-700">
                    ...
                    {!groupEntries.length && (
                      <p className="text-sm text-slate-500">No permissions available in catalog.</p>
                    )}
                  </div>
                </div>
              </div>
