...
export const CustomerDashboard: React.FC = () => {
  ...
  const canAccessBi = (user?.powerBiAccess || 'none') !== 'none';

  useEffect(() => {
    if (!user?.companyId) return;
    Promise.all([
      api.customer.getKPIs(user.companyId),
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Operational snapshot for your authenticated entity.</p>
        </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                Access level: <span className="font-mono">{user?.powerBiAccess || 'none'}</span>
              </p>
            </div>
            <Link
